...
var SimpleLogger = exports.SimpleLogger = {

  
  _loggers: {},
  getLogger: function SimpleLogger_getLogger(loggerName, filename, enableConsoleLogging, enableDumpLogging, considerNetLogging) {
    if (this._loggers[loggerName])
      return this._loggers[loggerName];
    if (!filename)
      filename = "chainsaw.ptr";
  
    let formatter = new Log4Moz.BasicFormatter();
    Log4Moz.repository.rootLogger.level = Log4Moz.Level.Debug;
  
    let theLog = Log4Moz.repository.getLogger(loggerName);
    theLog.level = Log4Moz.Level.Warn;
  
    try {
      // figure out if event-driven indexing should be enabled...
      let prefService = Cc["@mozilla.org/preferences-service;1"].
                          getService(Ci.nsIPrefService);
      let branch = prefService.getBranch("log4moz."+loggerName+".");
    ...
    if (enableConsoleLogging) {
      ...
      theLog.addAppender(capp);
    }
  
    if (enableDumpLogging) {
      let dapp = new Log4Moz.DumpAppender(formatter);
      dapp.level = Log4Moz.Level.All;
      theLog.level = Log4Moz.Level.All;
      theLog.addAppender(dapp);
    }
    ...
    if (considerNetLogging) {
      ...
      if (file.exists()) {
        ...
        theLog.level = Log4Moz.Level.All;
        theLog.addAppender(sapp);
      }
    }
    this._loggers[loggerName] = theLog;
    return theLog;
  },
